...
int main(int argc, char** argv)	{
	...
	bool massDblJetOrJetTau_DblJetOneIsoTau = true; // excludes two tau case, this seed is most like new vbf seed
	...
	if (passSel && printPrimL1) {
	    ...
	    std::vector<TLorentzVector> isoTau32L1Primitives;
            for (int iTau = 0; iTau < tauPrimSize; ++iTau) {
              ...
              if (tempTau_.Pt() >= 32 && inTree->tauL1PrimitivesIso->at(iTau)) isoTau32L1Primitives.push_back(tempTau_);
            }
            int isoTau32L1PrimitivesSize = isoTau32L1Primitives.size();

	    std::vector<TLorentzVector> isoTau40L1Primitives;
            std::vector<TLorentzVector> isoTau45L1Primitives;
	    for (int iTau = 0; iTau < isoTau32L1PrimitivesSize; ++iTau) {
                TLorentzVector tempTau_ = tauL1Primitives.at(iTau);
